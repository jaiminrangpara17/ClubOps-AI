...
export interface AuthContextValue {
  ...
  /** Creates an account, then signs in on success (backend-confirmed only). */
...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const register = useCallback(async (request: RegisterRequest) => {
    ...
    setNotice(null);
    try {
      // Backend-confirmed only: the mock adapter refuses registration, so this
      // path runs solely against a real POST /auth/register.
  ...
  const value = useMemo<AuthContextValue>(
    ...
    [session, status, isSubmitting, notice, login, register, logout, clearNotice],
  );
