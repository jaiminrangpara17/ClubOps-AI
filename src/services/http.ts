...
function resolveToken(explicit?: string): string | undefined {
  ...
  try {
    // Lazy require avoids issues if storage is unavailable (SSR/tests).
    const { loadSession } = sessionModule;
...
// Static import kept at bottom-of-file semantics via namespace object to make
// the dependency explicit without risking circular-import surprises.
