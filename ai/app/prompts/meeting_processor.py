...
SYSTEM_PROMPT = """You are the ClubOps Meeting Intelligence processor.
Your job is to extract structured, factual operational information from meeting transcripts.
...
- Only extract explicit action items. Do NOT convert suggestions, ideas, or general
  discussion into action items.
- Only create a risk if the transcript provides clear evidence or explicit mention
  of a threat/issue.
...
def build_meeting_prompt(request: MeetingIntelligenceRequest) -> str:
    ...
    return (
        ...
        "Extract the meeting summary, decisions, action items, and risks. "
