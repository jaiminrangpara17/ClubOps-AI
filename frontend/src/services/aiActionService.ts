...
async function executeInModuleServices(eventId: string, action: AiAction): Promise<AiAction> {
  ...
  switch (action.type) {
    ...
    case "update_risk": {
      ...
      const risk = await riskService.transitionRisk(eventId, riskId, proposed.status?.toLowerCase() as never);
    ...
    case "create_announcement": {
      const record = await announcementService.createAnnouncement(eventId, {
        ...
        priority: asTaskPriority(proposed.priority) as never,
...
const mockAiActionService: AiActionService = {
  ...
  async approveAction(eventId, actionId) {
    ...
    executed.updatedAt = nowIso();

    return { action: clone(executed) };
  },
