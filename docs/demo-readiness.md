...
In mock mode, sign in with the demo account supplied by your demo operator.
Mock credentials live only in the isolated mock auth adapter and are never
shown in the UI.

They must not be used or shown when `VITE_API_MODE=api`.
